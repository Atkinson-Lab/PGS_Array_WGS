...
# List of types and Phenotypes
types=("WGS")
phenotypes=("Height" "DBP" "HDL" "TC" "RBC" "leukocyte" "T2D" "Asthma" "Breast_Cancer" "Colorectal_Cancer")

# Loop over WGS type and Phenotypes
for type in "${types[@]}"; do
    for phenotype in "${phenotypes[@]}"; do
        # Define input and output files based on WGS type and Phenotype
        ...
        if [[ -f "$input_file" ]]; then
            ...
            NR > 1 {
                ...
                # Convert the value in $5 to numeric using +0
                neglog10_value = $5 + 0;  # Convert to numeric

                # Skip row if any value is "NA"
                if ($1 == "NA" || $2 == "NA" || $3 == "NA" || $4 == "NA" || $5 == "NA") {
                    next  # Skip this row if any field is "NA"
                }

                # Only calculate P if the value in $5 is numeric
                if (neglog10_value == neglog10_value) {  # Check if the value is a valid number
                    # If the value in column 5 is numeric, calculate P as 10 raised to the negative of that value
